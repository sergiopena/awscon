...
def get_instances():
  ...
  try:
    ...
    print e.message
    print "Make sure you defined and env var AWS_PROFILE pointing to your credentials"
    sys.exit(1)
  ...
  for ins in response['Reservations']:
    ...
    entry = "%s - %s - %s - %s " % (instanceId, '{:<15}'.format(publicAddress), '{:<15}'.format(privateAddress), name)
